...
const QaPanel: React.FC<Props> = ({ paperIds }) => {
  ...
  return (
    <div className="flex h-[calc(100vh-16rem)] flex-col rounded-lg border border-slate-200 bg-white">
      <div ref={listRef} className="flex-1 space-y-4 overflow-y-auto p-4">
        {records.length === 0 && !loading && (
          <div className="flex h-full flex-col items-center justify-center gap-2 text-slate-400">
            <Bot className="h-10 w-10" />
            <p className="text-sm">针对这篇论文提问，AI 基于论文内容回答</p>
          </div>
        )}
        {records.map(r => (
        ...
        {loading && (
          <div className="flex items-center gap-2 text-sm text-slate-400">
            <Loader2 className="h-4 w-4 animate-spin" /> AI 思考中...
          </div>
        )}
      </div>
      <div className="border-t border-slate-200 p-3">
        <div className="flex gap-2">
          <input value={question} onChange={e => setQuestion(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleAsk()}
            placeholder="输入你的问题..." className="flex-1 rounded-md border border-slate-300 px-3 py-2 text-sm focus:border-blue-500 focus:outline-none" />
